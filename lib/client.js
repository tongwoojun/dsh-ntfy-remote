...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * ntfy 单条 message 的字节硬上限。客户端 bundle 进不了宿主侧的 ntfy.js
     * （那边用了 Buffer），所以这里只作为输入框的 max 提示，真正的钳制在宿主。
     */
     ...
     *
     * 这两点都必须写清楚——按字理解会算错（一个汉字 3 字节），以为会截断会
     * 不敢调小，而实际行为是多发几条。
     */
    const MAX_MESSAGE_HINT = '单条推送正文的字节上限（一个汉字约 3 字节）。'
      + '超出会拆成多条继续发，内容不会丢；ntfy 硬上限 4095 字节，出厂默认 4000。'
    ...
    function SettingsSection() {
      ...
      return el('div', null,
        ...
        el('div', { style: S.row },
          ...
          el('span', null, '秒'),
          el('span', { style: { marginLeft: 12 }, title: MAX_MESSAGE_HINT }, '单条上限'),
          el('input', {
            type: 'number', min: 100, max: MAX_MESSAGE_BYTES, value: String(defaults.maxMessageLength),
            title: MAX_MESSAGE_HINT, 'aria-label': '单条推送正文上限（字节）',
